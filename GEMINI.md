...
★**報告する前に、まず `git push` してください。**
シロが確かめられるのは**リポジトリに届いた物だけ**です。手元でコミットしただけだと、報告を頂いてもシロ側の回答は「実装が見当たりません」になります（実際に4件がこの形でした）。報告の前に `git log origin/main..HEAD` が空である事を見てください。
...
7. **直したら、`git push` してから報告してください。**押し出していない修正は、シロからは存在しないのと同じです。コミットのshaを報告に書いても、そのshaがリポジトリに無ければ確かめようがありません。
